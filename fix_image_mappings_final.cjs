...
dests.forEach(dest => {
  ...
  // 2. Check wiki images
  else if (fs.existsSync(path.join(__dirname, 'public', 'images', 'destinations', `${slug}_wiki.jpg`))) {
    finalImagePath = `/images/destinations/${slug}_wiki.jpg`;
  }
  // 3. Check best images
  else if (fs.existsSync(path.join(__dirname, 'public', 'images', 'best', `${slug}.jpg`))) {
    finalImagePath = `/images/best/${slug}.jpg`;
  }
  // 4. Check fetched images
  ...
  // 6. Check old local images (if any)
